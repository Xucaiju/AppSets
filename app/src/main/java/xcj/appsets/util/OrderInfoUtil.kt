...
import xcj.appsets.util.AlipaySignUtil.sign
...
object OrderInfoUtil{
    ...
    fun getSign(
        ...
        val oriSign: String? = sign(authInfo.toString(), rsaKey, rsa2)
        Log.d("OriSign is null?","${oriSign==null}")
        var encodedSign = ""
        try {
    ...
    private val outTradeNo: String
        get() {
            val format =
                SimpleDateFormat("MMddHHmmss", Locale.getDefault())
            val date = Date()
