...
class AppSetsRecyclerSectionItemDecoration(
    context: Context,
    private val sectionCallback: SectionCallback,
    private val recyclerViewAttr: AppSetsRecyclerViewAttr
        ...
        thread (true) {
            childInContact?.let {
                ...
                if (getIsSection(contractPosition) && recyclerViewAttr.isSticky) {
                    val topChild = parent.getChildAt(0) ?: return@let
    ...
    private fun inflateHeaderView(parent: RecyclerView): View {
        return LayoutInflater.from(parent.context)
            .inflate(
                R.layout.appsets_time_line_recycler_section_header,
                parent,
                false
            )
    }
