...
class SearchResultSection(context: Context?, clickListener: ClickListener?) : InstallAppSection(context, clickListener) {
    ...
    override fun updateList(appList: MutableList<App>?) {
        this.appList?.clear()
        if (appList != null) {
            for (app in appList) {
